...
class MainActivity : SimpleActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        config.migrateStartTabToCallHistoryIfNeeded()
    }
        ...
        binding.mainTabsHolder.onGlobalLayout {
            Handler().postDelayed({
                ...
                if (intent.action == Intent.ACTION_VIEW && config.showTabs and TAB_CALL_HISTORY > 0) {
                    wantedTab = getTabIndex(TAB_CALL_HISTORY)
                }
    ...
    private fun getTabIcon(position: Int): Drawable {
        val drawableId = when (tabsList.getOrNull(position)) {
    ...
    private fun getTabLabel(position: Int): String {
        val stringId = when (tabsList.getOrNull(position)) {
    ...
    private fun getTabIndex(tab: Int): Int {
        var index = 0
        for (candidate in tabsList) {
            if (config.showTabs and candidate == 0) continue
            if (candidate == tab) return index
            index++
        }
        return 0
    }
    ...
    private fun getDefaultTab(): Int {
        return when (config.defaultTab) {
            TAB_LAST_USED -> {
                if (config.lastUsedViewPagerPage < binding.mainTabsHolder.tabCount) {
                    config.lastUsedViewPagerPage
                } else {
                    0
                }
            }
            TAB_CONTACTS, TAB_FAVORITES, TAB_CALL_HISTORY -> getTabIndex(config.defaultTab)
            else -> getTabIndex(TAB_CALL_HISTORY)
        }
    }
