...
class Config(context: Context) : BaseConfig(context) {
    companion object {
        ...
        private const val START_ON_CALL_HISTORY_MIGRATED = "start_on_call_history_migrated"
    }

    /** One-time: open Call History on launch (loads fast; contacts warm in background). */
    fun migrateStartTabToCallHistoryIfNeeded() {
        if (!prefs.getBoolean(START_ON_CALL_HISTORY_MIGRATED, false)) {
            defaultTab = TAB_CALL_HISTORY
            prefs.edit().putBoolean(START_ON_CALL_HISTORY_MIGRATED, true).apply()
        }
    }
