...
class CallService : InCallService() {
    override fun attachBaseContext(base: android.content.Context) {
        val locale = if (android.os.Build.VERSION.SDK_INT >= 33) {
            ...
            localeManager?.applicationLocales?.get(0) ?: java.util.Locale.getDefault()
        } else {
            if (base.config.useEnglish) java.util.Locale.ENGLISH else java.util.Locale.getDefault()
        }

        java.util.Locale.setDefault(locale)
        val configuration = base.resources.configuration
        configuration.setLocale(locale)
        val newContext = base.createConfigurationContext(configuration)
